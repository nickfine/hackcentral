...
**Current state (as of Feb 1, 2026):** v0.6.9. Phases 1–4 complete; hack types unified to prompts, skills, apps. Dashboard polish (8pt spacing, sticky header, content scroll) applied; PLAN_DASHBOARD_POLISH_2026.md documents design and done checklist. **Unified Hacks page:** Single "Hacks" nav item (FolderKanban) → `/hacks` with tabs Completed | In progress; `/library` and `/projects` redirect to `/hacks?tab=completed` and `/hacks?tab=in_progress` (query params preserved). Detail routes unchanged: `/library/:assetId`, `/projects/:projectId`. Display-only "Assets" → "Hacks" and Completed Hacks / Hacks In Progress labels in place; consistency review for unify-Hacks applied (Library close modal, pages export, QuickActionsPanel canonical URLs). See CODE_REVIEW_UNIFY_HACKS_CONSISTENCY.md and CODE_REVIEW_CONSISTENCY_INTEGRITY_ASSETS_TO_HACKS.md.
