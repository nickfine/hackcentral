...
**Current state (as of Feb 1, 2026):** v0.6.3. Phases 1–4 complete; hack types unified to prompts, skills, apps; Library/Projects renamed in UI to Completed Hacks / Hacks In Progress (routes /library and /projects unchanged). Display-only "Assets" → "Hacks" rename applied across UI (dual meaning: project + reusable item); code identifiers unchanged. Consistency & integrity review completed: PersonalizedNudge copy "Your hack has been reused", Library AssetCard badge "Featured Hacks" (was "Arsenal"); see CODE_REVIEW_CONSISTENCY_INTEGRITY_ASSETS_TO_HACKS.md.
