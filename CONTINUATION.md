...
- Curated runtime theme presets are now live in production:
  - Branding includes a 4-card preset selector: `Default`, `Editorial`, `Summit`, `Studio`
...
- Admin-only Branding transient preview is behaving correctly in production based on direct manual verification:
  - live preview updates immediately from the Branding tab
  - `Save branding` persists correctly
  - saved theme state remains applied while navigating across runtime pages
...
- Registration UX + localhost dev-controls cleanup is now live in production:
  - local dev `Event Phase` selector remains bound to the real/global phase instead of the derived participant phase
  - local `Participant - Needs Signup` impersonation now stays genuinely unregistered until signup completes
  - registration hero copy now makes signup the explicit next step
  - unregistered dashboard next action in `signup` now routes to signup with `Sign Up Now`
...
- Analytics consistency correction is now live in production:
  - `Signup Funnel` support counts now use `Welcome Views` so the displayed denominator matches `registerClickRate`
  - `Signup Completed` only turns concern/red after 48 hours when a real `registrationOpensAt` is published
...
- Runtime hero branding split is now live in production:
  ...
  - dashboard hero uses the banner as background and the icon in the hero mark area
  - custom hero banners render clear with no forced wash or opacity layer
  - Admin Branding preview cards are capped at `200px` and include explicit image guidance
  - latest production hosted validation artifact:
    - `docs/artifacts/staging-hero-split-validation-2026-03-09T00-08-53-237Z.json`
  - production child runtime route:
...
- Released a targeted analytics consistency follow-up to production.
- Version markers are now:
  - root app `0.6.80`
  - forge-native `0.3.58`
  - HackCentral UI marker unchanged at `0.6.66`
...
- Runtime/admin analytics behavior now:
  - aligns the Signup Funnel support row to `welcomeImpressions` / `Welcome Views`, matching the runtime denominator used by `registerClickRate`
  - removes the `startAt` fallback from the `Signup Completed` 48-hour concern rule
  - keeps zero completed signups neutral when `registrationOpensAt` is missing or invalid
...
- Local validation:
  ...
  - `cd /Users/nickster/Downloads/HackCentral/forge-native && ../scripts/with-node22.sh npm run custom-ui:build`
- Predeploy backup artifacts:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-152154Z.json`
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-152154Z.md`
- Deploy/install:
  - `../scripts/with-node22.sh forge deploy --environment production --no-verify`
  - `../scripts/with-node22.sh forge install -e production --upgrade --non-interactive --site hackdaytemp.atlassian.net --product confluence`
- Postdeploy production validation:
  - runtime iframe logged `[HackCentral Runtime v2] Module loaded - 1.2.92`
  - root attributes resolved to `data-color-mode=\"light\"` and `data-theme-preset=\"default\"`
  - runtime body loaded beyond the host shell and showed the live child runtime content
  - artifacts:
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T15-32-31-390Z.json`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T15-32-31-390Z.md`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T15-32-31-390Z.png`

### Current state

- Production Confluence is now running markers `0.6.80 / 0.3.58 / 1.2.92`.
- Ignore the earlier `2026-03-09T15-30-26-199Z` shell-only probe; the `2026-03-09T15-32-31-390Z` artifact trio is the authoritative postdeploy evidence for this release.
...
- Released the registration UX cleanup and localhost dev-controls fix to production.
- Version markers are now:
  - root app `0.6.76`
  - forge-native `0.3.54`
  - HackCentral UI marker unchanged at `0.6.66`
...
- Runtime/dev behavior now:
  - keeps the local Event Phase dropdown tied to the real/global phase
  - makes `Participant - Needs Signup` a true transient unregistered simulation until signup completes
  - updates registration-phase messaging so the next step clearly directs users to signup
  - removes the redundant free-agent warning panel from the dashboard hero
...
- Local validation:
  ...
  - `./scripts/with-node22.sh npm run build --prefix forge-native/static/runtime-frontend`
- Predeploy backup artifacts:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-132223Z.json`
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-132223Z.md`
- Deploy/install:
  - `../scripts/with-node22.sh npm run custom-ui:build`
  - `../scripts/with-node22.sh forge deploy --environment production --no-verify`
  - `../scripts/with-node22.sh forge install -e production --upgrade --non-interactive --site hackdaytemp.atlassian.net --product confluence`
- Postdeploy production validation:
  - runtime iframe logged `[HackCentral Runtime v2] Module loaded - 1.2.88`
  - root attributes resolved to `data-color-mode="dark"` and `data-theme-preset="default"`
  - dashboard hero was present on the live child runtime
  - artifacts:
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T13-26-11-818Z.json`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T13-26-11-818Z.md`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T13-26-11-818Z.png`

### Current state

- Production Confluence is now running markers `0.6.76 / 0.3.54 / 1.2.88`.
- The registration hero and next-action flow now push unregistered users toward signup more clearly.
- The local dev controls now support the `Registration` + `Participant - Needs Signup` test path reliably.
...
- Released a small follow-up so production markers and continuity docs match the verified production state.
- Version markers are now:
  - root app `0.6.74`
  - forge-native `0.3.52`
  - HackCentral UI marker unchanged at `0.6.66`
...
- No functional runtime behavior was intentionally changed in this release beyond the version-marker bump and continuity-note correction.
...
- Local validation:
  ...
  - `./scripts/with-node22.sh npm run build --prefix forge-native/static/runtime-frontend`
- Predeploy backup artifacts:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-110719Z.json`
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-110719Z.md`
- Deploy/install:
  - `../scripts/with-node22.sh npm run custom-ui:build`
  - `../scripts/with-node22.sh forge deploy --environment production --no-verify`
  - `../scripts/with-node22.sh forge install -e production --upgrade --non-interactive --site hackdaytemp.atlassian.net --product confluence`
- Postdeploy production validation:
  - runtime app-shell logged `[HackCentral Runtime v2] Module loaded - 1.2.86`
  - root attributes resolved to `data-color-mode="light"` and `data-theme-preset="default"`
  - artifacts:
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T11-08-52-143Z.json`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T11-08-52-143Z.md`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/release-version-check-2026-03-09T11-08-52-143Z.png`

### Current state

- Production Confluence is now running markers `0.6.74 / 0.3.52 / 1.2.86`.
- The transient Branding preview remains treated as working in production based on direct manual verification.
- The earlier root-attribute caveat should not be reopened unless a fresh user-facing regression is reproduced.

## Session Update - v0.6.73 Admin Branding Transient Preview Released (Mar 9, 2026 10:52 GMT)
...
- Runtime Branding now:
  - shows a compact `SAVED` / `UNSAVED` badge next to `Theme preset`
  - disables `Save branding` when there are no pending Branding changes
  - uses an admin-only transient preview path instead of writing live Branding edits into Config Mode draft state
...
- Local validation:
  ...
  - `./scripts/with-node22.sh npm run build --prefix forge-native/static/runtime-frontend`
- Predeploy backup artifacts:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-104436Z.json`
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-104436Z.md`
- Deploy/install:
  - `../scripts/with-node22.sh npm run custom-ui:build`
  - `../scripts/with-node22.sh forge deploy --environment production --no-verify`
  - `../scripts/with-node22.sh forge install -e production --upgrade --non-interactive --site hackdaytemp.atlassian.net --product confluence`
- Hosted production validation:
  - runtime app-shell logged `[HackCentral Runtime v2] Module loaded - 1.2.85`
  - Branding badge flow verified `SAVED` -> `UNSAVED` -> `SAVED`
  - `Save branding` button verified disabled -> enabled -> disabled across the same flow
  - artifacts:
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/admin-branding-transient-preview-production-2026-03-09T10-49-07-761Z.json`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/admin-branding-transient-preview-production-2026-03-09T10-49-07-761Z.md`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/admin-branding-transient-preview-production-2026-03-09T10-49-07-761Z-unsaved.png`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/admin-branding-transient-preview-production-2026-03-09T10-49-07-761Z-reverted.png`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/admin-branding-transient-preview-production-2026-03-09T10-49-07-761Z-summary.json`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/admin-branding-transient-preview-production-2026-03-09T10-49-07-761Z-summary.md`

### Current state

- Production Confluence is now running the admin Branding transient-preview release with markers `0.6.73 / 0.3.51 / 1.2.85`.
- Hosted smoke confirms the new Branding `SAVED` / `UNSAVED` UX and save-button gating are live.
- Manual production verification confirms the transient preview itself is also working correctly.
- Code path reminder:
  - [`App.jsx`](/Users/nickster/Downloads/HackCentral/forge-native/static/runtime-frontend/src/App.jsx) prefers `adminBrandingPreview` over persisted branding while the Admin view is active
  - [`useTheme.js`](/Users/nickster/Downloads/HackCentral/forge-native/static/runtime-frontend/src/hooks/useTheme.js) applies `data-theme-preset` directly to `document.documentElement`
- The earlier root-attribute caveat is best understood as a test artifact from a fragile hosted probe that hit runtime/frame readiness issues before the full Admin surface was available.
...
- Released the curated runtime theme preset system to production.
- Version markers are now:
  - root app `0.6.71`
  - forge-native `0.3.49`
  - HackCentral UI marker unchanged at `0.6.66`
...
- Runtime branding now supports:
  - `themePreset` persistence and normalization with fallback to `default`
  - root-level `data-theme-preset` application independent from `data-color-mode`
  - shared-surface preset token bundles for `default`, `editorial`, `summit`, and `studio`
  - Branding preview cards that switch the live preview immediately without saving
...
- Predeploy backup artifacts:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-013658Z.json`
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-P10-PREDEPLOY-BACKUP-active-events-20260309-013658Z.md`
- Deploy/install:
  - `../scripts/with-node22.sh npm run custom-ui:build`
  - `../scripts/with-node22.sh forge deploy --environment production --no-verify`
  - `../scripts/with-node22.sh forge install -e production --upgrade --non-interactive --site hackdaytemp.atlassian.net --product confluence`
- Hosted production validation:
  - runtime app-shell logged `[HackCentral Runtime v2] Module loaded - 1.2.83`
  - root attributes resolved to `data-color-mode="light"` and `data-theme-preset="default"`
  - Branding rendered 4 preset cards and preview switched to `Summit` immediately on selection
  - artifacts:
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/theme-preset-production-validation-2026-03-09T01-38-26-888Z.json`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/theme-preset-production-validation-2026-03-09T01-38-26-888Z.md`
    - `/Users/nickster/Downloads/HackCentral/docs/artifacts/theme-preset-production-validation-2026-03-09T01-38-26-888Z.png`

### Current state

- Production Confluence is running the curated theme preset release with markers `0.6.71 / 0.3.49 / 1.2.83`.
- Existing events without a stored `themePreset` now fall back to `default`.
- Release process note: if runtime package versions are bumped, also update `forge-native/static/runtime-frontend/src/data/constants.js` so the console/version marker matches the deployed runtime bundle.
...
- Released the split runtime hero-branding changes to production.
- Version markers now live in production:
  - root app `0.6.69`
  - forge-native `0.3.47`
  - HackCentral UI marker unchanged at `0.6.66`
...
- Production runtime branding now includes:
  - separate `Hero banner image` and `Hero icon image` upload controls
  - `bannerImageUrl` for the hero background and `heroIconImageUrl` for the hero mark
  - clear custom banner rendering with no added wash or opacity layer
  - `200px` preview caps plus explicit banner/icon guidance copy
- Production deploy/install executed exactly per [`DEPLOY.md`](/Users/nickster/Downloads/HackCentral/DEPLOY.md):
  - predeploy backup snapshots:
  ...
  - Forge install result: production site already at latest version after upgrade
- Hosted production validation passed on [Shona's IT Hack](https://hackdaytemp.atlassian.net/wiki/apps/f828e0d4-e9d0-451d-b818-533bc3e95680/86632806-eb9b-42b5-ae6d-ee09339702b6/hackday-app?pageId=24510466) with `/Users/nickster/Downloads/HackCentral/.auth/hackdaytemp-storage.json`:
  - runtime app-shell logged `[HackCentral Runtime v2] Module loaded - 1.2.81`
  - Branding showed two file inputs
  - uploaded banner matched dashboard background
  - uploaded icon matched dashboard hero logo area
  - returning to Branding before publish preserved both uploaded previews
  - artifacts:
...
- Deployed the current local runtime hero-branding split changes to Forge `staging` using the staging path from [`DEPLOY.md`](/Users/nickster/Downloads/HackCentral/DEPLOY.md):
  - `../scripts/with-node22.sh npm run custom-ui:build`
...
- Staging environment confirmed at:
  - environment id `17778174-f2aa-4f73-a34a-985afd5fa4e2`
  - child runtime route: [staging HackDay child page](https://hackdaytemp.atlassian.net/wiki/apps/f828e0d4-e9d0-451d-b818-533bc3e95680/17778174-f2aa-4f73-a34a-985afd5fa4e2/hackday-app?pageId=24510466)
- Hosted iframe validation passed with `/Users/nickster/Downloads/HackCentral/.auth/hackdaytemp-storage.json`:
  - runtime app-shell logged `[HackCentral Runtime v2] Module loaded - 1.2.80`
  - Branding tab showed separate `Hero banner image` and `Hero icon image` controls
  - banner upload produced a `.../branding/banner-...` URL and updated the dashboard hero background immediately in Config Mode
  - icon upload produced a `.../branding/icon-...` URL and replaced the dashboard hero logo area immediately in Config Mode
  - returning to `Admin Panel -> Branding` without publishing preserved both uploaded previews
  - reusable QA commands now available:
    - `npm run qa:runtime:branding:staging`
    - `npm run qa:runtime:branding:staging:deploy`
  - artifact bundle:
