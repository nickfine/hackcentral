...
- Post-seed verification artifact:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-MIGRATION-DRY-RUN-hdc-auto-20260218-150620Z.md`
  - key deltas:
    - `Total submitted hacks: 1` (was `0`)
    - canonical prod instance now reports `submitted hacks: 1, total projects: 1`.
- Updated launch-readiness artifact to reflect gate closure:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-LAUNCH-READINESS-20260218-1352Z.md`
  - recommendation updated to **GO (release-ready on canonical baseline)**.

**Phase 5 resiliency deploy + seed retry checkpoint P (Feb 18, 2026 14:22 UTC):**
- Promoted seed-resiliency hardening to production:
  - `forge deploy --non-interactive -e production` ✅ (`4.6.0`)
  - `forge install --upgrade ... -e production` ✅
  - `forge install list` confirms production remains `Up-to-date` on `hackdaytemp`.
- Retried canonical seed with stable title:
  - payload target event: `075f09ae-1805-4a88-85bc-4cf43b03b612`
  - result: explicit bounded-retry failure surfaced by new logic:
    - `Seed hack failed after rate-limit retries: Supabase POST Project failed (429): Too Many Requests`
- Verification artifact after retry:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-MIGRATION-DRY-RUN-hdc-auto-20260218-142222Z.md`
  - state unchanged (`Total submitted hacks: 0`).
- Status:
  - conditional-go gate remains open on upstream Supabase rate limiting; internal retry/idempotency handling is now live.

**Phase 5 seed resiliency hardening checkpoint O (Feb 18, 2026 14:18 UTC):**
- Added seed-flow resilience while waiting on Supabase rate-limit recovery:
  - `seed_hack` now retries rate-limited writes (`429`/`Too Many Requests`) with bounded backoff.
  - seed action is now idempotent by `(eventId, hackTitle)` and returns `seeded=false, skipped=true` when duplicate title already exists.
- Added ops regression coverage:
  - `/Users/nickster/Downloads/HackCentral/tests/forge-native-phase5-ops.spec.ts`
  - covers: missing eventId validation, no-admin blocker, idempotent skip behavior, and 429 retry-success path.
- Validation:
  - `npm -C /Users/nickster/Downloads/HackCentral run test:run -- tests/forge-native-phase5-ops.spec.ts tests/forge-native-repository-event-config.spec.ts tests/forge-native-repository-sync.spec.ts tests/forge-native-hdcService.spec.ts tests/phase5-performance-harness.spec.ts` ✅ (`38` tests passing)
- Deploy/install:
  - not deployed in this checkpoint (code/test hardening only).

**Legacy cleanup checklist artifact added (Feb 18, 2026 14:06 UTC):**
- Added actionable legacy/historical cleanup checklist with owner/action/verification/unblock criteria:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-LEGACY-CLEANUP-CHECKLIST-20260218-1406Z.md`
- Checklist tracks three remaining conditions:
  1. manual Confluence orphan page archive/delete (`6029333`, `5767177`),
  2. canonical migration target naming normalization (`HackDay 2026` vs `HDC Auto*`),
  3. seed submission closure retry after Supabase `429` window clears.

**Phase 5 seed-attempt blocker checkpoint N (Feb 18, 2026 14:01 UTC):**
- Attempted to close the remaining readiness condition by seeding one hack into canonical production event:
  - target event: `075f09ae-1805-4a88-85bc-4cf43b03b612` (`HDC Auto 1771412434287`)
- Implemented operational seed path in Forge ops handler:
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/ops.ts`
  - action `seed_hack` now uses existing event admin user ID and repository seed insert path (no new user creation).
- Added backend helper for controlled seeding:
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/backend/supabase/repositories.ts`
  - `seedHackForEventAsUser(...)`
- Added team-schema compatibility hardening for legacy Team variants:
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/backend/supabase/repositories.ts`
  - `ensureLegacyTeamRecord`/`defaultTeamFieldValue` now support both `id` and `teamId` key shapes.
- Observed blocker during live seed attempts (multiple retries/backoff):
  - Supabase API returns non-JSON `Too Many Requests` payloads / HTTP `429` on write path.
- Hardened Supabase client parsing for non-JSON error bodies:
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/backend/supabase/client.ts`
  - avoids JSON parse crash and preserves original upstream error text/status.
- Current verification artifact after seed attempts:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-MIGRATION-DRY-RUN-hdc-auto-20260218-140147Z.md`
  - result remains: `No submitted hacks: 2` / `Total submitted hacks: 0`.
- Status impact:
  - readiness remains **Conditional GO** with outstanding operational blocker: Supabase write throttling prevents seed submission closure step.

**Phase 5 launch-readiness matrix checkpoint M (Feb 18, 2026 13:52 UTC):**
- Completed Phase 5 final readiness pass using production dry-run evidence and current canonical instances.
- Added launch-readiness artifact:
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-LAUNCH-READINESS-20260218-1352Z.md`
  - includes:
    - canonical target resolution status,
    - migration integrity summary,
    - performance baseline summary,
    - final QA matrix,
    - ops command set,
    - go/no-go recommendation with explicit conditions.
- Improved dry-run artifact hygiene:
  - `/Users/nickster/Downloads/HackCentral/scripts/phase5-migration-dry-run.mjs`
  - output filenames now include query slug + timestamp to prevent collisions in consecutive runs.
- New canonical dry-run artifacts with unique names:
  ...
  - `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-MIGRATION-DRY-RUN-hdc-auto-20260218-135230Z.md` (`2` matches, integrity checks pass)
- Remaining gating condition captured:
  - production currently has no event matching `HackDay 2026`; active canonical instances remain `HDC Auto*`.
  - full migration sign-off still requires canonical target row identification/provisioning and at least one submitted hack in target instance for realistic sync-volume validation.

**Phase 5 migration dry-run checkpoint L (Feb 18, 2026 13:48 UTC):**
- Implemented production-safe migration dry-run tooling via Forge webtrigger (runs in Forge runtime with encrypted Supabase variables).
- New backend ops entrypoint:
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/ops.ts`
  - computes per-event migration integrity checks:
    - Confluence page/parent presence
    - admin role distribution
    - sync state presence/status
    - submitted hack/project counts
    - audit row counts
  - returns rollback checklist for apply-failure recovery.
- Added migration query helpers:
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/backend/supabase/repositories.ts`
  - `listMigrationEventCandidatesByName(...)`
  - `countEventAuditLogs(...)`
- Manifest/runtime wiring:
  - `/Users/nickster/Downloads/HackCentral/forge-native/manifest.yml`
  - added webtrigger module `phase5-migration-wt` backed by function `phase5-migration-ops`
  - configured `response.type: dynamic` for Supabase egress support.
- Added local execution script:
  - `/Users/nickster/Downloads/HackCentral/scripts/phase5-migration-dry-run.mjs`
  - npm command:
    - `/Users/nickster/Downloads/HackCentral/package.json` -> `qa:phase5:migration-dry-run`
- Production deploy/install for ops endpoint:
  - `forge deploy --non-interactive -e production` ✅ (`4.2.0`)
  - `forge install --upgrade ... -e production` ✅ (latest on `hackdaytemp`)
- Artifacts generated:
  - requested target query (`HackDay 2026`): `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-MIGRATION-DRY-RUN-20260218-134821Z.md`
    - result: `0` matching events (naming/data-target gap)
  - active-canonical baseline query (`HDC Auto`): `/Users/nickster/Downloads/HackCentral/docs/artifacts/HDC-V2-PHASE5-MIGRATION-DRY-RUN-20260218-134844Z.md`
    - result: `2` matching events, page IDs/parent IDs/admin/sync-state integrity checks all pass; both have `0` submitted hacks.
- Validation rerun:
  - `npm --prefix /Users/nickster/Downloads/HackCentral/forge-native run typecheck` ✅
  - `npm -C /Users/nickster/Downloads/HackCentral run test:run -- tests/forge-native-repository-event-config.spec.ts tests/forge-native-repository-sync.spec.ts tests/forge-native-hdcService.spec.ts tests/phase5-performance-harness.spec.ts` ✅ (`34` tests passing)

**Phase 5 stress/performance harness checkpoint K (Feb 18, 2026 13:36 UTC):**
- Implemented Phase 5 stress/perf harness with configurable thresholds and repeatable output.
- New test harness:
  - `/Users/nickster/Downloads/HackCentral/tests/phase5-performance-harness.spec.ts`
  - scenarios:
    - `registry_lookup` scale test (`listAllEvents`) at `2500` events by default,
    - `complete_and_sync` high-volume test (`completeAndSync`) at `3000` hacks by default.
  - threshold env controls:
    - `HDC_PERF_REGISTRY_EVENTS`
    - `HDC_PERF_REGISTRY_ITERATIONS`
    - `HDC_PERF_REGISTRY_P95_MS`
    - `HDC_PERF_SYNC_HACKS`
    - `HDC_PERF_SYNC_ITERATIONS`
    - `HDC_PERF_SYNC_P95_MS`
- Added npm command:
...
- Deploy/install outcomes:
  - no deploy/install actions executed in this checkpoint (test harness + baseline capture only).

**Phase 5 instrumentation deploy + signal verification checkpoint J (Feb 18, 2026 13:33 UTC):**
- Promoted Phase 5 kickoff instrumentation to both Forge environments and verified install state on `hackdaytemp`.
- Validation/build gates before deploy:
  - `npm --prefix /Users/nickster/Downloads/HackCentral/forge-native run typecheck` ✅
  - `npm --prefix /Users/nickster/Downloads/HackCentral/forge-native run frontend:build` ✅
  - `npm --prefix /Users/nickster/Downloads/HackCentral/forge-native run macro:build` ✅
  - `npm -C /Users/nickster/Downloads/HackCentral run test:run -- tests/forge-native-repository-event-config.spec.ts tests/forge-native-repository-sync.spec.ts tests/forge-native-hdcService.spec.ts` ✅ (`32` tests passing)
- Telemetry signal quality verification:
  - runtime test execution emitted `[hdc-performance-telemetry]` records for `metric=registry_lookup` with source, duration, rowCount, fallback flag, and success outcome.
  - sync telemetry emission remains covered by assertions in `tests/forge-native-hdcService.spec.ts` (`completeAndSync` success/error paths).
- Deploy/install outcomes:
  - `forge deploy --non-interactive -e development` ✅ (`5.32.0`)
  - `forge install --upgrade ... -e development` ✅ (already latest)
  - `forge deploy --non-interactive -e production` ✅ (`3.24.0`)
  - `forge install --upgrade ... -e production` ✅ (already latest)
  - `forge install list` confirms both environments `Up-to-date` on `hackdaytemp.atlassian.net`.

**Phase 5 instrumentation kickoff checkpoint I (Feb 18, 2026 13:23 UTC):**
- Started Phase 5 migration/performance readiness instrumentation from `main` at commit `b4e3e8f`.
- Added backend performance telemetry for registry lookup timing:
  - `listAllEvents`
  - `listEventsByParentPageId`
  - telemetry event shape emitted under `[hdc-performance-telemetry]` with metric `registry_lookup`, source, durationMs, outcome, rowCount, and legacy-fallback flag.
- Added sync execution telemetry for duration and error-rate tracking:
  - `HdcService.completeAndSync`
  - `HdcService.retrySync`
  - telemetry event shape emitted under `[hdc-performance-telemetry]` with metric `sync_execution`, action, durationMs, outcome, sync status/category, retryable flag, and warning payload.
- Files updated:
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/backend/supabase/repositories.ts`
  - `/Users/nickster/Downloads/HackCentral/forge-native/src/backend/hdcService.ts`
...
## Team Detail UX Remediation promotion checkpoint (2026-02-20 10:51 UTC)

### Scope
- Executed only the next pending slice after the `2026-02-20 01:45 UTC` validation boundary: HackDay 2026 Team Detail Page UX remediation and production promotion.
- Scope remained presentation-layer only (no Supabase schema/model changes).

### Exact changes
1. Team Detail two-zone restructure + contextual CTA contract:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/TeamDetail.jsx`
   - removed redundant `Team Detail` label,
   - promoted capacity indicator (`x/y members` + fill bar),
   - replaced lower submission CTA block with single contextual header CTA (`You're on this team` / `Request to Join` / `Leave Team`),
   - moved `Delete Idea` behind overflow menu,
   - added typed confirmation gate (`Type "<team name>" to confirm`) before delete button enables,
   - grouped editable sections into one workspace card with section dividers,
   - added helper placeholders:
     - `Describe what you're building in one sentence`
     - `What user problem does this solve? Be specific`
     - `Links, resources, or additional context for judges and voters`
   - switched to inline pencil icon edit affordance,
   - elevated Team Members above content cards,
   - made skills prominent per member,
   - added hybrid skill-gap signal (`inferred roles + team.lookingFor`),
   - changed pending requests empty state to collapsed single line (`No pending requests`) and expandable when count `> 0`.

2. App-level leave-team wiring:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/App.jsx`
   - added `handleLeaveTeam` resolver path and passed `onLeaveTeam` into Team Detail.

3. Top-bar context dedupe for Team Detail only:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/AppLayout.jsx`
   - added `showTeamContextInHeader` prop and suppressed team-context block on team-detail route while preserving other pages.

4. Countdown label hardening + phase-target semantics:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/AppLayout.jsx`
   - countdown label now phase-humanized (`until Team Formation begins/closes`, etc.),
   - removed dependence on raw event-name tokens in timer label,
   - added robust epoch-string parsing to avoid raw timestamp leakage.

5. Phase stepper visual state strengthening:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/ui/PhaseIndicator.jsx`
   - stronger active highlight,
   - explicit check icon for completed phases,
   - muted future phase treatment.

6. Test coverage updates:
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/local/team-detail-ux.spec.ts`
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/local/smoke.spec.ts`
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/confluence/shared/smoke.spec.ts`
   - added assertions for overflow delete gate and raw countdown-leak absence.

7. Release tracking + built assets:
   ...
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/dist/*` refreshed.

### Exact versions
- HD26Forge app package: `7.5.59 -> 7.5.60`
- HD26Forge custom UI package: `1.2.33 -> 1.2.34`
- Forge production deploy line on `hackdaytemp`: `5.58.0 -> 5.60.0`

### Validation results
- `/Users/nickster/Downloads/HD26Forge/static/frontend`: `npm run build` ✅ (`hackday-custom-ui@1.2.34`)
- `/Users/nickster/Downloads/HD26Forge`: `npm run test:e2e:local` ✅ (`4/4`)
- `/Users/nickster/Downloads/HD26Forge`: `E2E_CONFLUENCE_URL=... npm run test:e2e:confluence` ✅ (`3/3`)
- `/Users/nickster/Downloads/HD26Forge`: `npm run qa:health:prod` ✅ (`PASS: macro invocations and production log scan are clean`)
- Note: pre-deploy confluence smoke correctly failed on old production line due expected raw-label leak; post-deploy rerun passed.

### Deploy/install outcomes
- `/Users/nickster/Downloads/HD26Forge`: `forge deploy -e production --non-interactive` ✅
  - deployed app version: `5.60.0`
  - latest successful production deploy row: `2026-02-20T10:49:14.621Z`
- `/Users/nickster/Downloads/HD26Forge`: `forge install --upgrade --site hackdaytemp.atlassian.net --product confluence -e production --non-interactive` ✅
  - result: `Site is already at the latest version`
- install verification:
...
### Commit hash(es)
- Implemented + promoted release commit (HD26Forge `main`): `7a65604`
- Push status: `origin/main` updated `045823d..7a65604`.

## Continuation update (2026-02-20 10:53 UTC)

### Scope
- Closed the next pending slice after the `2026-02-20 10:51 UTC` checkpoint: progress-doc synchronization and rollback evidence confirmation.
- No additional product code changes were made in HD26Forge in this slice.

### Exact changes
1. Added this timestamped checkpoint to all required HackCentral progress artifacts:
   ...
   - `/Users/nickster/Downloads/HackCentral/docs/HDC-HACKDAY-TEMPLATE-SPINOUT-PLAN.md`
2. Confirmed rollback anchor remains the promoted HD26Forge `main` commit.

### Exact versions
- HD26Forge app package: `7.5.60`
- HD26Forge custom UI package: `1.2.34`
- Forge production deploy line on `hackdaytemp`: `5.60.0`

### Validation results
- No new validation commands were required for this docs-only closure slice.
- Authoritative validation remains from `2026-02-20 10:51 UTC`:
  - local custom-ui build ✅
  - local e2e ✅ (`4/4`)
  - confluence e2e post-deploy ✅ (`3/3`)
  - production health check ✅ (`PASS`)

### Deploy/install outcomes
- No new deploy/install operations were executed in this docs-only slice.
- Authoritative deploy/install state remains:
  - `forge deploy -e production --non-interactive` succeeded with deployed version `5.60.0`
  - `forge install --upgrade ... -e production --non-interactive` reported site up-to-date
  - install list still indicates `App version 5`, `Up-to-date`

### Commit hash(es)
- No new HD26Forge commit in this docs-only slice.
- Rollback target remains HD26Forge `main` commit: `7a65604`.

## Continuation update (2026-02-20 12:05 UTC)

### Scope
- Closed the next pending slice after the `2026-02-20 10:53 UTC` checkpoint: Team Detail pass-2 visual polish and structural refinement.
- Scope remained presentation-layer only (no Supabase schema/model changes).

### Exact changes
1. Team Detail pass-2 UX polish:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/TeamDetail.jsx`
   - compressed identity header into two-row layout,
   - moved captain/member presence badge inline with team-name row,
   - removed instructional subtitle and redundant capacity label,
   - replaced generic team icon with initials avatar,
   - slim semantic capacity bar (green/amber/red thresholding),
   - rebalanced content zone to 40/60 desktop split (`Team Members` then `Workspace` on mobile),
   - unified card styling (`rounded-xl`, `border`, `shadow-sm`, consistent padding),
   - merged pending requests into Team Members card,
   - removed nested-box treatment for Looking For and standardized pills,
   - refined member rows (name/captain/callsign/skills rhythm),
   - standardized workspace sections with always-visible edit pencils,
   - updated placeholder guidance strings for Goal/Problem/More Info.
2. Phase-stepper visual-state refinement:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/ui/PhaseIndicator.jsx`
   - stronger complete/active/future differentiation,
   - active ring/pulse emphasis,
   - complete/future connector treatment (solid vs dashed).
3. E2E coverage updates:
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/local/team-detail-ux.spec.ts`
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/local/smoke.spec.ts`
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/confluence/shared/smoke.spec.ts`
   - added pass-2 contract assertions and mobile stack verification.
...
### Exact versions
- HD26Forge app package: `7.5.60 -> 7.5.61`
- HD26Forge custom UI package: `1.2.34 -> 1.2.35`
- Forge production deploy line on `hackdaytemp`: `5.60.0 -> 5.61.0`

### Validation results
- `/Users/nickster/Downloads/HD26Forge/static/frontend`: `npm run build` ✅ (`hackday-custom-ui@1.2.35`)
...
### Commit hash(es)
- HD26Forge release commit (main): `07fd5e2`
- Push status: `origin/main` updated `7a65604..07fd5e2`.

## Continuation update (2026-02-20 13:29 UTC)

### Scope
- Closed the next pending slice after the `2026-02-20 12:05 UTC` checkpoint: Team Detail final polish (pass 3) plus production promotion and verification.
- Scope remained presentation-layer only (no Supabase schema or backend contract changes).

### Exact changes
1. Team Detail final polish implementation:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/TeamDetail.jsx`
   - swapped columns to workspace-first (`60%`) and members-second (`40%`) on desktop; mobile stack is workspace then members,
   - strengthened deterministic initials avatar styling,
   - placed `x/y members` and capacity bar on a single row,
   - differentiated captain vs membership status badges,
   - removed visible `Team Workspace` heading (kept semantic `sr-only`),
   - shortened section label to `Problem to Solve`,
   - normalized section rhythm/edit icon polish,
   - softened members heading and pending zero-state,
   - aligned member metadata row (`Captain` + skills together),
   - added bottom anchor footer (`Last updated ...` + `Back to all teams`).
2. Phase-stepper final state polish:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/components/ui/PhaseIndicator.jsx`
   - clearer active emphasis and muted future state treatment in compact modes.
3. Local mock/e2e contract alignment:
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/src/App.jsx`
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/local/team-detail-ux.spec.ts`
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/local/smoke.spec.ts`
   - `/Users/nickster/Downloads/HD26Forge/tests/e2e/confluence/shared/smoke.spec.ts`
   - updated assertions for workspace-first order, section-label semantics, badge differentiation, same-row capacity and footer anchor.
4. Release metadata/build outputs:
   - `/Users/nickster/Downloads/HD26Forge/package.json` + lock
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/package.json` + lock
   - `/Users/nickster/Downloads/HD26Forge/static/frontend/dist/*` refreshed.

### Exact versions
- HD26Forge app package: `7.5.61 -> 7.5.62`
- HD26Forge custom UI package: `1.2.35 -> 1.2.36`
- Forge production deploy line on `hackdaytemp`: `5.61.0 -> 5.62.0`
...
- `/Users/nickster/Downloads/HD26Forge`: `forge deploy -e production --non-interactive` ✅
  ...
  - latest successful production deploy row: `2026-02-20T13:25:40.888Z`
- `/Users/nickster/Downloads/HD26Forge`: `forge install --upgrade --site hackdaytemp.atlassian.net --product confluence -e production --non-interactive` ✅
...
### Commit hash(es)
- HD26Forge release commit on `main`: `30382c0`
- Push status: `origin/main` advanced `07fd5e2..30382c0`.
...
## Dashboard visual energy pass notes (2026-02-20 23:21 UTC)

- For visual-only dashboard passes, keep a small set of reusable motion-safe utility classes in `/Users/nickster/Downloads/HD26Forge/static/frontend/src/index.css` (`@media (prefers-reduced-motion: no-preference)`) and consume them by class name (`dashboard-hero-cta-glow`, `activity-item-stagger`, `phase-active-beacon`) instead of adding one-off inline animation logic.
- Deterministic avatar color variety is easy to stabilize for tests by hashing display names to a fixed class palette; this preserves visual variety while keeping repeatable e2e assertions.
- In a non-deploy slice, Confluence e2e that asserts newly-added `data-testid` contracts against production pages can fail even when local build + local e2e pass; for true Confluence verification of new selectors, deployment (or a Confluence environment pinned to the new bundle) is required.

## Dashboard theming parity notes (2026-02-20 23:43 UTC)

- For reliable light/dark parity, avoid hardcoded `bg-white`, `bg-gray-*`, `text-gray-*` in dashboard shells; use the same token-backed classes as Team Detail (`bg-arena-card`, `bg-arena-elevated`, `border-arena-border`, `text-text-primary|secondary|muted`).
- If a component needs semantic visual states across themes (readiness pills, activity accents, live chip, timeline badges), prefer named CSS contract classes in `index.css` (for example `dashboard-readiness-pill--amber`) with `[data-color-mode="dark"]` overrides instead of tailwind-only color literals.
- When refactoring `PhaseIndicator`, keep chevron fill variables aligned with segment state; removing the shared fill constant without updating the chevron style causes runtime `ReferenceError` and breaks app boot.
